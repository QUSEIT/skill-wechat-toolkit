 ...
 *
 * 主题支持:
 *   - 内置: default, lapis, phycat
 *   - 自定义: aurum (金色主题), openclaw (橘红主题)
 */
...
console.log('🔍 aurum CSS 路径:', path.join(BASE_DIR, 'references', 'aurum-theme.css'));
console.log('🔍 aurum CSS 存在:', fs.existsSync(path.join(BASE_DIR, 'references', 'aurum-theme.css')));
...
const THEMES = {
    ...
    'aurum': { 
        type: 'custom', 
        css: path.join(BASE_DIR, 'references', 'aurum-theme.css') 
    },
    'openclaw': { 
        type: 'custom', 
        css: path.join(BASE_DIR, 'references', 'openclaw-theme.css') 
    },
    // 别名支持
    'gold': { type: 'custom', css: path.join(BASE_DIR, 'references', 'aurum-theme.css') },
    '金色': { type: 'custom', css: path.join(BASE_DIR, 'references', 'aurum-theme.css') },
    '橙色': { type: 'custom', css: path.join(BASE_DIR, 'references', 'openclaw-theme.css') },
    '橘红': { type: 'custom', css: path.join(BASE_DIR, 'references', 'openclaw-theme.css') },
    '红色': { type: 'custom', css: path.join(BASE_DIR, 'references', 'openclaw-theme.css') },
};
...
function resolveTheme(themeName) {
    console.log(color.yellow(`🔍 解析主题: ${themeName}`));
    
    const normalized = themeName.toLowerCase().trim();
    const theme = THEMES[normalized] || THEMES[themeName];
    
    console.log(color.yellow(`🔍 主题映射结果:`, theme));
    
    if (!theme) {
        // 未知主题，回退到内置主题
        console.log(color.yellow(`🔍 未知主题，回退到内置主题: ${themeName}`));
        return { type: 'builtin', name: themeName, args: ['-t', themeName] };
    }
    
    if (theme.type === 'builtin') {
        console.log(color.yellow(`🔍 使用内置主题: ${themeName}`));
        return { 
            type: 'builtin', 
            name: themeName, 
            args: ['-t', themeName] 
    ...
    // 自定义主题
    console.log(color.yellow(`🔍 CSS 文件路径: ${theme.css}`));
    console.log(color.yellow(`🔍 CSS 文件是否存在: ${fs.existsSync(theme.css)}`));
    
    if (!fs.existsSync(theme.css)) {
        console.error(color.red(`❌ 主题文件不存在: ${theme.css}`));
        process.exit(1);
    }
...
// ─── 发布函数 ────────────────────────────────────────────────
function publish(file, themeName, highlight, appId, secret) {
    ...
    if (theme.type === 'custom') {
        console.log(color.cyan(`  使用自定义主题: ${theme.css}`));
    }

    const env = { ...process.env, WECHAT_APP_ID: appId, WECHAT_APP_SECRET: secret };
    
    // 构建 wenyan 命令参数
    const args = ['publish', '-f', file, '-h', highlight, ...theme.args];
    
    console.log(color.yellow(`🔍 执行命令: wenyan ${args.join(' ')}`));
...
Examples:
  ...
  node publish.js article.md aurum solarized-light
  node publish.js article.md openclaw solarized-light

Available themes:
  内置主题: default, lapis, phycat
  自定义主题: aurum (金色), openclaw (橘红)
  
  支持别名: gold, 金色, 橙色, 橘红, 红色
